...
def plot_accuracy(
    excel_path=r"C:\Users\Administrator\Desktop\Accuracy.xlsx",
    sheet_name=0,
    out_dir="./figures"
):
    ...
    plt.close()
    print(" 已保存到：", (out_dir / "accuracy_models.pdf").resolve())
    print(" 已保存到：", (out_dir / "accuracy_models.png").resolve())
...
def _load_pooled_truth_probs(model_dir):
    """
    读取某个模型目录下按迭代保存的 y_true_iter*.npy 与 y_prob_iter*.npy，
    纵向拼接为一个整体样本池，返回 y_true_all, y_prob_all
    """
    y_trues, y_probs = [], []
    for j in range(1, 22):
        ...
        if yt_p.exists() and yp_p.exists():
            ...
            print(f"[提示] 缺少 {yt_p.name} 或 {yp_p.name}，已跳过。")
    if not y_trues:
        return None, None
...
def plot_roc_pr_for_models(
    ...
    画多模型 ROC 与 PR 对比图
    参数:
      model_dirs: {模型显示名: 保存 y_true_iter*.npy / y_prob_iter*.npy 的目录}
      out_dir: 输出目录
    """
    out_dir = Path(out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)

    # 与你已有的顺序/配色/marker保持一致
    order   = ["T-LGB", "Adaboost", "Gbrt", "RandomForest", "XGBoost"]
    ...
    for name, color, mk in zip(order, colors, markers):
        ...
        if y_true is None:
            print(f"[跳过] {name} 没有可用的 y_true/y_prob 文件")
            continue
        fpr, tpr, _ = roc_curve(y_true, y_prob)
        roc_auc = auc(fpr, tpr)
        plt.plot(fpr, tpr, color=color, linewidth=1.8)
        # 标注一个点（例如 TPR≈0.8 处的最近点）
    ...
    print("✅ 已保存：", (out_dir / "roc_models.png").resolve())
    ...
    print(" 已保存：", (out_dir / "pr_models.png").resolve())
