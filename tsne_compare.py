...
SC_PATH       = r"D:\hainan\回归\sc\sample\5站总\samples1.npy"       
EC_LOW_PATH   = r"D:\hainan\回归\ec_low总\samples1.npy"              
EC_HIGH_PATH  = r"D:\hainan\回归\feature\feature1.npy"               
LABEL_PATH    = r"D:\fog_prediction\data\ec高空\ViT_label\总站\低\label\label1.npy"
...
def tsne3d(
    X, y, ax, title,
    seed=RANDOM_SEED, perplexity=PERPLEXITY, lr=LEARNING_RATE, max_iter=MAX_ITER,
    use_pca=USE_PCA_BEFORE_TSNE, pca_n=PCA_N_COMPONENTS
):
    ...
    X3 = tsne.fit_transform(X_in)  # (N, 3)
...
def main():
    ...
    assert sc.shape[0] == N and eclow.shape[0] == N and echigh.shape[0] == N, \
        f"样本数不一致: labels={N}, sc={sc.shape[0]}, eclow={eclow.shape[0]}, echigh={echigh.shape[0]}"
    ...
    plt.show()
    print(f"[SAVE] 3D t-SNE 对比图已保存：{out_png}")
